...
When('The user enters valid Login credentials', async ({loginPageFixture}) => {
    await loginPageFixture.enterUsername(process.env.username);
    await loginPageFixture.enterPassword(process.env.password);
  });
...
Then('The user sees Login error message {string}', async function ({loginPageFixture}, errorMessage) {
    const message = await loginPageFixture.getLoginErrorMessage();
